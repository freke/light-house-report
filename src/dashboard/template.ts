...
function buildMetricsRow(metrics: any, url: string, prefix: string): string {
  ...
  const fcp = metrics.fcp == null ? '-' : (metrics.fcp / 1000).toFixed(2) + 's';
  const lcp = metrics.lcp == null ? '-' : (metrics.lcp / 1000).toFixed(2) + 's';
  const tbt = metrics.tbt == null ? '-' : metrics.tbt.toFixed(0) + 'ms';
  const cls = metrics.cls == null ? '-' : metrics.cls.toFixed(3);
  const si = metrics.si == null ? '-' : (metrics.si / 1000).toFixed(2) + 's';
  const tti = metrics.tti == null ? '-' : (metrics.tti / 1000).toFixed(2) + 's';
...
export function generateHtml(statsData: StatsData): string {
  ...
  const trendData = urls.map((url) => {
    const urlLabel = statsData.urls[url].label;
    const modesTrend: Record<string, any[]> = {};
    for (const mode of allModes) {
      modesTrend[mode] = (statsData.urls[url].modes[mode] || []).map((r: any) => ({
        x: r.timestamp,
